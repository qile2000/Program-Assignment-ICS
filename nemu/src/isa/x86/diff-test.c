...
bool isa_difftest_checkregs(CPU_state *ref_r, vaddr_t pc) {
  ...
  if ( ref_r->edx != cpu.edx){/*printf("ref.edx:%x  cpu.edx:%x  ref.ebx:%x  cpu.ebx:%x\n",ref_r->edx,cpu.edx,ref_r->ebx,cpu.ebx);return false;}*/printf("pc at %x ref %x: ref.edx: %d, cpu.edx: %d\n",cpu.pc,ref_r->pc,ref_r->edx,cpu.edx); return false;}
  ...
  printf("pc at %x:\n",cpu.pc);
  //printf("ref.edx:%x  cpu.edx:%x  ref.ebx:%x  cpu.ebx:%x\n",ref_r->edx,cpu.edx,ref_r->ebx,cpu.ebx);
  return true;
}
